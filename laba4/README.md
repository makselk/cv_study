...
Время выполнения функций сравнивалось при помощи std::chrono, поочередно скармливая одну и ту же картинку разным функциям. По результатам сравнения самым быстрым оказался встроенный алгоритм, примерно в ~200 раз быстрее. Наивный метод и cooley-turkey в данной реализации находятся примерно на одном уровне по времени выполнения. Такая большая разница может быть объяснена лишними переводами типов данных, а таже сильной неоптимизированности кода (много неоптимальных копирований). 

Кроме того, также были сравнены совпадения полученных спектров (если разница в значении не превышает одного процента - совпадают). Результат лобового метода и cooley-turkey максимально совпадает с результатом полученным с помощью функции opencv.
...
- <ins>По шаблону найти на изображении фурье глаза фурье используя корреляцию изображений по образам фурье</ins>

Немного подправил предыдущую функцию: добавил вычитание среднего значения из исходного изображения и шаблона. Теперь только после этого производится корреляция. Таким образом получилось добиться большей робастности еще и в предыдущем задании. Первый скрин - glaza.png, второй - glaz.png.

![Пример работы программы](readme_img/example10_0.png)
![Пример работы программы](readme_img/example10_1.png)
